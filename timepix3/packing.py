...
# set masks for the bit packet
ftoaM     = 0b1111                
totM      = 0b1111111111          
toaM      = 0b11111111111111  
dummy10bM = 0b1111111111
addrM     = 0b1111111111111111    
headerM   = 0b1111                

# convert encoded counter values to 48-bit packet in ToA & ToT Mode (op_mode=00):

def pack_001(header,addr,toa,tot,ftoa): 
    
    packet =  ((header&headerM)<<44)+((addr&addrM)<<28)+((toa&toaM)<<14)+((tot&totM)<<4)+(ftoa&ftoaM)
   
    return packet


# convert encoded counter values to 48-bit packet in ToA Only Mode (op_mode=01)

def pack_011(header,addr,toa,dummy10b,ftoa): 
    
    packet = ((header&headerM)<<44)+((addr&addrM)<<28)+((toa&toaM)<<14)+((dummy10b&dummy10bM)<<4)+(ftoa&ftoaM)
   
    return packet


# write list of packets to binary output file:

def packets_to_file(filename, packets):
...
def values_to_file(df,file, op_mode=0):
    ...
    # loop through rows of data frame and create packets:
    packets=np.arange(len(toa), dtype='int64')
    for i in packets:
        if op_mode==0:
            packets[i]=pack_001(header,addr.iloc[i],toa.iloc[i],tot.iloc[i],ftoa.iloc[i])
        elif op_mode==1:
            packets[i]=pack_011(header,addr.iloc[i],toa.iloc[i],dummy.iloc[i],ftoa.iloc[i])
    
    # write to file:
    packets_to_file(file, packets)
...
def file_to_packets(filename):
    ...
    packets=np.arange(n, dtype='int64')
